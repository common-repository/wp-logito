...
function LogitoDeliveryMan_method_init() {
	if ( ! class_exists( 'WC_LogitoDeliveryMan' ) ) {
		class WC_LogitoDeliveryMan extends WC_Shipping_Method {
			...
			public function __construct() {
				...
				$this->id                 = 'logito_delivery_man';
				$this->method_title       = __( 'ارسال پیک', 'Logito' );
				$this->method_description = __( 'ارسال پیک شرکت لجیتو', 'Logito' ); //
				...
				$this->title   = isset( $this->settings['title'] ) ? $this->settings['title'] : __( 'ارسال پیک', 'Logito' );
			...
			function init_form_fields() {
				...
				$this->form_fields = array(
					...
					'enabled' => array(
						...
						'type'        => 'checkbox',
						'description' => __( 'فعال کردن روش ارسال ارسال پیک شرکت لجیتو', 'Logito' ),
						'default'     => 'yes'
					),
					...
					'title'                                => array(
						...
						'default'     => __( 'ارسال پیک', 'Logito' )
					),
					'discountByShipment'                   => array(
						'title'   => __( 'تخفیف براساس شیوه ارسال پیک', 'Logito' ),
						'type'    => 'select',
					...
					'discountByShipmentValue'              => array(
						'title'       => __( 'مقدار تخفیف براساس شیوه ارسال پیک', 'Logito' ),
						'type'        => 'price',
						'placeholder' => wc_format_localized_price( 0 ),
						'default'     => '0',
						'description' => 'در صورتی در "تخفیف براساس شیوه ارسال پیک" گزینه "رایگان" را انتخاب کرده اید لازم نیست این فیلد را ویرایش کنید، در صورتی که گزینه "تخفیف درصدی" را انتخاب کرده اید در صد تخفیف را وارد کنید و در صورتی که "تخفیف مقداری" را انتخاب کرده اید براساس ارز وبسایت مبلغ تخفیف را وارد نمایید.',
					),
			...
			public function calculate_shipping( $package = array() ) {
				...
				$cartData = array(
					...
					"SendTypeId"    => intval( LogitoCore::$LogitoDeliveryManCode ),
				...
				if ( WC()->session->get( 'DeliveryManCartShippingCostData' ) == $cartData ) {
					Debug::Log( WC()->session->get( 'DeliveryManCartShippingCostRate' ), "Shipment costs returns from session" );

					// Set delivery amount
					$rate = $this->calculateDiscount( $package, WC()->session->get( 'DeliveryManCartShippingCostRate' ) );
					$this->add_rate( $rate );
				...
				if ( $shippingCosts->getResponseCode() == 0 ) {
					...
					$this->calculateDiscount( $package, $rate );

					// Set retrieved shipping data in users session
					WC()->session->set( 'DeliveryManCartShippingCostData', $cartData );
					WC()->session->set( 'DeliveryManCartShippingCostRate', $rate );
					...
					WC()->session->set( 'DeliveryManCartShippingCostData', null );
	...
	function add_LogitoDeliveryMan_shipping_method( $methods ) {
		// Move to top
		array_unshift( $methods, 'WC_LogitoDeliveryMan' );

		return $methods;
	}

	// Define shipment method to woocommerce
	add_filter( 'woocommerce_shipping_methods', 'add_LogitoDeliveryMan_shipping_method' );
}

add_action( 'woocommerce_shipping_init', 'LogitoDeliveryMan_method_init' );