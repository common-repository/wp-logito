...
function LogitoCertifiedMail_method_init() {
	if ( ! class_exists( 'WC_LogitoCertifiedMail' ) ) {
		class WC_LogitoCertifiedMail extends WC_Shipping_Method {
			...
			public function __construct() {
				...
				$this->id                 = 'logito_certified_mail';
				$this->method_title       = __( 'پست سفارشی', 'Logito' );
				$this->method_description = __( 'پست سفارشی شرکت لجیتو', 'Logito' ); //

				$this->enabled = "yes";

				$this->enabled = isset( $this->settings['enabled'] ) ? $this->settings['enabled'] : 'yes';
				$this->title   = isset( $this->settings['title'] ) ? $this->settings['title'] : __( 'پست سفارشی', 'Logito' );
			...
			function init_form_fields() {
				$this->form_fields = array(
					...
					'enabled' => array(
						...
						'type'        => 'checkbox',
						'description' => __( 'فعال کردن روش ارسال پست سفارشی شرکت لجیتو', 'Logito' ),
						'default'     => 'yes'
					),
					...
					'title'                                => array(
						...
						'default'     => __( 'پست سفارشی', 'Logito' )
					),
					'discountByShipment'                   => array(
						'title'   => __( 'تخفیف براساس شیوه ارسال پست سفارشی', 'Logito' ),
						'type'    => 'select',
					...
					'discountByShipmentValue'              => array(
						'title'       => __( 'مقدار تخفیف براساس شیوه ارسال پست سفارشی', 'Logito' ),
						'type'        => 'price',
						'placeholder' => wc_format_localized_price( 0 ),
						'default'     => '0',
						'description' => 'در صورتی در "تخفیف براساس شیوه ارسال پست سفارشی" گزینه "رایگان" را انتخاب کرده اید لازم نیست این فیلد را ویرایش کنید، در صورتی که گزینه "تخفیف درصدی" را انتخاب کرده اید در صد تخفیف را وارد کنید و در صورتی که "تخفیف مقداری" را انتخاب کرده اید براساس ارز وبسایت مبلغ تخفیف را وارد نمایید.',
					),
			...
			public function calculate_shipping( $package = array() ) {
				...
				$cartData = array(
					...
					"SendTypeId"    => intval( LogitoCore::$LogitoCertifiedMailCode ),
				...
				if ( WC()->session->get( 'CertifiedMailCartShippingCostData' ) == $cartData ) {
					Debug::Log( WC()->session->get( 'CertifiedMailCartShippingCostRate' ), "Shipment costs returns from session" );

					$rate = $this->calculateDiscount( $package, WC()->session->get( 'CertifiedMailCartShippingCostRate' ) );
				...
				if ( $shippingCosts->getResponseCode() == 0 ) {
					...
					WC()->session->set( 'CertifiedMailCartShippingCostData', $cartData );
					WC()->session->set( 'CertifiedMailCartShippingCostRate', $rate );
					...
					WC()->session->set( 'CertifiedMailCartShippingCostData', null );
	...
	function add_LogitoCertifiedMail_shipping_method( $methods ) {
		// Move to top
		array_unshift( $methods, 'WC_LogitoCertifiedMail' );

		return $methods;
	}

	// Define shipment method to woocommerce
	add_filter( 'woocommerce_shipping_methods', 'add_LogitoCertifiedMail_shipping_method' );
}

add_action( 'woocommerce_shipping_init', 'LogitoCertifiedMail_method_init' );